...
`,
	...
	// Wait for all three, not for the last of them. The phase does not arrive
	// as one batch — the replay a subscription is opened with and the pair the
	// first GET /next publishes are separate deliveries — and each of them has
	// its own cut, its own retry and its own way of being late. So "initReport
	// has arrived" is not evidence about the records published before it, and
	// waiting on one of the three to then assert the other two were already
	// there is a race — the one this test lost in #1437. Waiting for all three
