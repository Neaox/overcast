...
export interface CopyUrlButtonProps {
  ...
  noun?: string
  defaultIndex?: number
  /** Icon-only mode for tight spaces like table row actions. */
  compact?: boolean
}
...
export function CopyUrlButton({ formats, noun, defaultIndex = 0, compact }: CopyUrlButtonProps) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-0.5 text-fg-muted", compact && "gap-0")}>
      {!compact && (
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild>
            <button
              type="button"
              className={cn(
                "flex items-center gap-0.5 rounded px-1 py-0.5 min-w-0",
                "hover:bg-bg-muted transition-colors",
                "focus-visible:ring-2 focus-visible:ring-accent focus-visible:outline-none",
              )}
              title={active.label}
            >
              <span className="truncate font-mono text-xs max-w-64">
                {active.value}
              </span>
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content
              ...
              sideOffset={4}
              className={cn(
                "z-50 min-w-[14rem] rounded-md border border-border bg-bg-elevated p-1 shadow-lg",
              )}
              ...
              {formats.map((f, i) => (
                <DropdownMenu.Item
                  ...
                  className={cn(
                    ...
                    "text-fg outline-none hover:bg-accent hover:text-white focus:bg-accent focus:text-white",
      ...
      {compact ? (
        <DropdownMenu.Root>
          <span className="inline-flex items-center">
            <button
              type="button"
              onClick={() => copy(active.value, { noun: noun ?? active.label })}
              title={`Copy ${noun ?? active.label}`}
              aria-label={`Copy ${noun ?? active.label}`}
            ...
            <DropdownMenu.Trigger asChild>
              <button
                type="button"
                className="inline-flex items-center justify-center h-7 w-4 rounded hover:bg-bg-muted transition-colors -ml-0.5"
                title="Choose format"
                aria-label="Choose URL format"
              >
