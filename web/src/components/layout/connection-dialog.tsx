...
function ValidationIcon({ state }: { state: ValidationState }) {
  switch (state.kind) {
    case "checking":
      return (
        <Tooltip content="Checking endpoint…" side="top">
          <Loader2 className="h-4 w-4 animate-spin text-fg-muted" />
        </Tooltip>
      )
    case "valid":
      return (
        <Tooltip content={`Overcast ${state.version} — reachable`} side="top">
          <Check className="h-4 w-4 text-success" />
        </Tooltip>
      )
    case "invalid":
      return (
        <Tooltip content={state.reason} side="top">
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        return { ok: false as const, error: e instanceof Error ? e.message : "Unreachable" }
      }
    },
...
export function ConnectionForm({
  ...
  const endpointUnknown =
    typeof window !== "undefined" && window.__OVERCAST__?.endpointKnown === false
  ...
  const labelPlaceholder = (() => {
    try {
      return new URL(baseUrlDraft).host
    } catch {
      return "Local (4566)"
    }
  })()
      ...
      {inDocker && endpointUnknown && (
        <div className="mb-5 flex items-start gap-3 rounded-lg border border-warning/30 bg-warning/10 px-4 py-3">
          ...
          <p className="text-sm text-fg-muted">
            ...
            Overcast cannot determine the host API port automatically. Mount the socket (
            <code className="rounded bg-bg px-1 font-mono text-xs">
              -v /var/run/docker.sock:/var/run/docker.sock
            </code>
            ) for automatic detection, or enter the API endpoint manually below.
          </p>
        </div>
      )}
      <form
        ...
        <div className="mt-2 flex flex-col gap-2">
          <form.Subscribe selector={(s) => [s.canSubmit, s.isSubmitting]}>
            {([canSubmit, isSubmitting]) => (
              <Button
                type="submit"
                className={cn(fullWidthSubmit ? "w-full" : "self-start")}
                disabled={!canSubmit}
              >
...
export function ConnectionDialogContent({ onSubmitted }: { onSubmitted?: () => void } = {}) {
  return (
    <div className="w-full max-w-md rounded-xl border border-border bg-bg-elevated p-8 shadow-2xl">
      ...
      <ConnectionForm
        fullWidthSubmit
        footnote="Settings are stored locally in this browser."
  ...
  return (
    ...
    >
      <div onClick={(e) => e.stopPropagation()}>
        <ConnectionDialogContent onSubmitted={onClose} />
      </div>
    </div>
  )
}
