...
mailRoutes.get("/messages", async (c) => {
  ...
  const url = `${ep.baseUrl}/_overcast/ses/inbox/messages${limit ? `?limit=${limit}` : ""}`
...
mailRoutes.get("/messages/:id", async (c) => {
  const ep = endpoint(c)
  const res = await fetch(`${ep.baseUrl}/_overcast/ses/inbox/messages/${c.req.param("id")}`)
...
mailRoutes.delete("/messages", async (c) => {
  const ep = endpoint(c)
  const res = await fetch(`${ep.baseUrl}/_overcast/ses/inbox/messages`, { method: "DELETE" })
