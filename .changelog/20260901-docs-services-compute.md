...
* [docs/scheduler] the cron section described L, W, # and the three-letter month and day names as unsupported, and never mentioned that day-of-week is AWS 1-7 from Sunday.
  Scheduler has shared EventBridge parser internal/awscron for several releases, so those expressions are accepted and evaluated
* [docs/cloudformation] the resource-type counts read 127 provisioned and 9 stubs of 136 registered, matching the resourceHandlers map; the page previously said 126/10 in one place and 132 in another
* [docs/lambda] the CDK examples target port 4566 rather than 2456, which is not a port Overcast ever listens on
* [docs/eventbridge] ECS RunTask is listed as a target for pattern-matched rules, not only scheduled ones, and Auto Scaling joins the list of services that publish their own events onto the default bus
* [docs/pipes] LogConfiguration and KmsKeyIdentifier are described as discarded rather than stored-but-inert: neither field exists on the pipe, so DescribePipe never returns them
