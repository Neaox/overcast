...
### A4 — CloudWatch Logs: range + limit pushdown for `GetLogEvents`/`FilterLogEvents`  **[✅ done — with pagination G1/G6]**
...
**Done (2026-07-24, branch feat/logs-range-and-tokens):** `eventBackend` gained `getEventsRange`/`getGroupEventsRange` on both backends, driving the previously unused `idx_logs_events_group` index; `memEventBackend` now assigns a persistent per-stream seq at append time (never derived from length — the A1 lesson applied preemptively) so both backends share one `(Timestamp, Seq)`/`(Timestamp, StreamName, Seq)` total order. Buffer merge (P4) fetches exactly `limit` persisted rows — provably sufficient, since buffered events can only displace persisted ones out of the final trimmed page, never require reaching further back (proof in code comment). Benchmark: FilterLogEvents allocs/op flat at 45 from 100 → 1,000,000 events/stream at a fixed window. Accepted memory-mode-only caveat: the memory backend discovers a group's streams via map scan before merging, so cost grows with stream *count* (bounded metadata) — the SQL backend is a single indexed query. Bonus: both ops' JSON-1.1 handlers now delegate to the typed implementations (previously a second, still-broken copy served the default SDK wire path).
...
### A6 — S3: `ListObjects` pages internally instead of materializing the bucket  **[✅ done — with pagination G4]**
