...
> **Status:** in progress — H1/H2/G3 (foundations), G1/G6 (CloudWatch Logs, with storage-access A4), G2/G5 (DynamoDB, with A3), G4 (S3, with A6), G9 (AppSync) all done; remaining: wave-3 items G7 (EC2), G8 (breadth) — re-verified still open 2026-08-21 (EC2 has no `NextToken` scaffolding, IAM still hardcodes `IsTruncated: false`, Kinesis `HasMoreStreams` still hardcoded). Audit completed 2026-07-24 (agent sweep of every paginating operation across all supported services; evidence file:line below).
...
### G4 — S3 `ListParts` / `ListMultipartUploads`: response shape is missing truncation fields entirely  **[✅ done — with storage-access A6]**
...
**Done (2026-07-24, branch feat/s3-paging):** full truncation-field sets added to both XML envelopes (defaults/caps 1000 per AWS docs); `NextPartNumberMarker` kept as a wire integer and `KeyMarker`/`UploadIdMarker` as plain values with AWS's compound resume rule (`Key > KeyMarker OR (Key == KeyMarker AND UploadId > UploadIdMarker)`) — deliberately NOT `serviceutil.Paginate`/H1, whose opaque token would break SDK XML decoding and client-constructed markers (the "adapt, don't shoehorn" case this plan predicted). Malformed `part-number-marker` errors with `InvalidArgument` instead of silently resetting to part 1. Both ops stay in-memory-paginated per the boundedness rule (parts ≤10k, uploads bounded metadata). Failing-first: all four new contract tests failed pre-fix (params ignored, no truncation fields, wrong same-key semantics). The v2 invalid-token fix landed with A6 (see that item's note).
...
### G9 — AppSync default-limit divergence  **[✅ done]**

[handler.go:138-185](../../internal/services/appsync/handler.go) already validates tokens and caps `maxResults` at 25 — but when the client omits it, default = everything. AWS default is 25. One-line default; keep their helper (see migration policy). Landed as one `maxResultsLimit` constant serving both the cap and the omitted-parameter default, so the two cannot drift.
...
| **P2 — small, high-visibility fidelity** | **G5** (DynamoDB ListTables — ✅ done), **G4** (S3 multipart response shapes + v2/v1 token errors — ✅ done), **G9** (AppSync default — ✅ done) | Cheap, mechanical, on surfaces users actually script against; each is an afternoon with H1/H2 in place. |
