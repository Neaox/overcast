...
> are still scoped to six backends with `rust-sdk` excluded, because #1885
> (the rust emitter cannot spell a composite nested inside a structure
> literal) is still open; every other wave-1 and pilot group lists all seven.
>
...
> composites, above; a fix is open as #1890, not yet merged), #1886 (`$name`
...
> is 360 operations). **The snapshot/budget PR is open as #1891** (not yet
...
11. **Two G4 wave 1 follow-ups remain open, both filed 2026-09-06, neither
    ...
    fix is open as #1890, not yet merged. #1878: `rust-sdk` needs an
