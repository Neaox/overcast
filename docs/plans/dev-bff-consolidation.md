...
**Follow-up, same day: the per-service files joined the manifest.** `web/src/types/topology.ts` and `mail.ts` are deleted and `trace.ts` keeps only the client-side request types (`TraceListParams`, `TraceSearchParams`) plus `TraceEvent = StreamEvent` (the by-request events endpoint serves the live stream's envelope); everything they described is now generated — 17 more manifest entries (`router.topologyResponse/Node/Edge`, `smtp.CapturedMessage/MessageKind`, the `trace` package's `LogEntry/Summary/OmitReason/Match/MatchField/DeepResult/RetentionStats/DroppedCounts`, and the trace list page). `cloudformation.ts` stays: it re-exports AWS SDK types, which is the right source for those. Two Go-side changes made that honest rather than approximate: `trace.Entry`/`Hop` marshal through function-local `shadow` structs (bodies as strings, derived `*BodyTruncated` booleans), so those are hoisted to package-level `entryJSON`/`hopJSON` and **tsgen now refuses any manifest type that has its own `MarshalJSON`** — rendering the in-memory struct would have been a confidently wrong contract; and the trace list page was a `map[string]any` literal, now `router.debugTraceListResponse`. Also `net/http.Header` → `Record<string, string[]>`, an empty-string constant (the zero value `omitempty` elides) is not a union member, and `topologyNode.ECSResourceType` / the ECS node constants got a typed alias so the `"cluster" | "service" | "task"` union is Go-owned. Drift surfaced: `CapturedMessage.kind` is required (the "legacy stored messages" the TS comment guarded against do not exist — the inbox is an in-memory ring and every constructor sets Kind), `TraceSummary.region` and `TopologyNode.domainName`/`originCount` were on the wire and missing from TS.

Two honest limits of the renderer remain, documented in its package comment: nil slices/maps marshal as `null` but are rendered as plain arrays/records (every manifest handler sends empty collections, several say so), and embedded/anonymous-struct fields are rejected rather than flattened until a consumed struct needs them.
