...
> **Status:** IMPLEMENTED (2026-07-25) — phase 1 numeric encoding (#283), phase 2 index structure (#287), phase 3 read-path flip + Transact maintenance + projection fidelity (#300). All deferred follow-ups from §5/§7 are now closed: LSI routing fix, ConsistentRead-on-GSI ValidationException, and parallel-scan segmentation all DONE (2026-08-03, #488). Originally a design-first doc, gated per [storage-access-plan.md A7](./storage-access-plan.md#a7--dynamodb-gsi-query-secondary-index); the reviewer question at the bottom was resolved in favour of Option A as specified.
...
| Parallel scan (`TotalSegments > 1`), no index | Same branch, same evidence | Even a plain parallel Scan (no `IndexName`) falls into this branch (`req.TotalSegments <= 1` is required for the A3 fast path at line 644) — full scan + full sort, then a positional slice per segment. **Closed 2026-08-03** by the §5 segmentation follow-up ([scan_segments.go](../../internal/services/dynamodb/scan_segments.go)). |
...
- **LSIs get no new structure.** A Local Secondary Index shares the base table's hash key by definition — an LSI Query is already expressible as a partition-scoped read (`scanItemsByHashKey`, the same O(k) primitive base-table Query already uses) filtered/sorted by the LSI's sort key, with no need for a *separately hash-partitioned* ordered structure the way a GSI needs. Real AWS also forbids adding/removing LSIs after table creation (`updateTableTyped` has no `LocalSecondaryIndexUpdates` handling today, matching AWS), so there's no backfill question either. **DONE (2026-08-03, the separable routing follow-up):** `queryTyped`'s LSI case now calls `scanItemsByHashKey` on the base partition and filters by the LSI's sort key, instead of sharing GSI's old full-table-scan branch. The fix also closed a sparseness bug the routing exposed — an LSI's *sort* key is the only key attribute an item can lack (the hash key is the table's own, always present), so both LSI Query and the GSI/LSI Scan fallback now exclude items missing the index sort key, matching AWS's "only tracks data items where its key attributes actually exist" rule. Items whose index key is absent were previously returned by an LSI read.
...
- **Parallel scan segments over an index or over the base table.** Segmenting an *ordered* structure correctly is its own design question — real DynamoDB defines segment boundaries by internal partition/hash-space division, which Overcast has no equivalent physical concept of. This was out of scope for A7 itself and **DONE (2026-08-03) as the separable follow-up**, in [scan_segments.go](../../internal/services/dynamodb/scan_segments.go): segment membership is `FNV-1a(storage-encoded partition key) mod TotalSegments` — the table's partition key for a base-table scan, the index's own partition key for a GSI scan, which is the closest analogue of AWS's hash-space division and keeps all items sharing a partition key in one segment as AWS does. Because membership is a pure function of the item's own key, segments are disjoint and complete by construction *and* stable while the table is being written to; the old full-scan-then-positional-slice derived boundaries from the item **count**, so any concurrent insert re-sliced the list and migrated existing items across segment boundaries — a worker-per-segment client saw duplicates or gaps. The read is now a bounded keyset walk over the same `scanPage`/`scanIndexPage` primitives A3 and the GSI flip use (no full materialization, no Go sort, page-sized rather than table-sized), and a parallel GSI scan reads the index structure, so it is projection-faithful and sparse-correct exactly like the unsegmented GSI scan — it previously fell through to the base-table fallback and could return attributes outside the index's projection. LSI scans keep the full-scan fallback (no LSI index storage exists) but use the same key-hash segment assignment, so one segmentation rule holds whatever is being scanned. A dedicated per-segment ordered structure would let a segment seek straight to a contiguous range, but it needs its own storage and migration; the walk reuses what exists and is bounded by page size rather than table size, which is the cost this bullet flagged.
...
4. **Follow-up, explicitly separable (not blocking A7 acceptance):** LSI query routing fix (§5), `ConsistentRead=true`-on-GSI `ValidationException` (§2), parallel-scan-over-ordered-structure design (§5). **All three DONE (2026-08-03)** — see the status line at the top and the §2/§5 entries.
