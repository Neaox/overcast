...
> **Status:** IMPLEMENTED (2026-07-25) — phase 1 numeric encoding (#283), phase 2 index structure (#287), phase 3 read-path flip + Transact maintenance + projection fidelity (this PR). All deferred follow-ups from §5/§7 are now closed: LSI routing fix, ConsistentRead-on-GSI ValidationException, and parallel-scan segmentation all DONE (2026-08-03). Originally: design-first, gated per [storage-access-plan.md A7](./storage-access-plan.md#a7--dynamodb-gsi-query-secondary-index) — no code in this PR. Reviewer verdict requested at the bottom.
