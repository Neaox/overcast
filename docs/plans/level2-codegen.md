...
**What's right.** `internal/protocol/codec` (Codec interface, `JSON10/JSON11/QueryXML/RESTXML/RPCv2CBOR`, precision-ordered `DefaultIdentifiers()`, `middleware.Protocol` stashing `(codec, opName)` in context) and `internal/protocol/op` (`Typed[In,Out]`, `TypedAny`, `NewRaw`) are sound, well-documented, byte-stability-disciplined. Error envelopes are fully centralized (`protocol.Write{JSON,XML,QueryXML,EC2QueryXML}Error`); no hand-rolled AWS error envelopes exist in service code. Eight services are cleanly typed-only (appconfig, appconfigdata, appregistry, appsync, eks, msk, opensearch, scheduler). Bedrock was a ninth until #857: bedrock-runtime is modeled `restJson1` and nothing else, so its typed registry served an `X-Amz-Target` prefix and a Smithy RPC v2 endpoint no AWS client addresses, and it was deleted rather than migrated.
...
>
> **Rider resolved (2026-07-26, branch fix/malformed-json-serialization-exception):** the variants were right. Real AWS JSON-protocol services reject an unparseable body with 400 `SerializationException` (DynamoDB/coral: `__type com.amazon.coral.service#SerializationException`; Smithy's restJson1 malformed-request protocol tests pin 400 + `x-amzn-errortype: SerializationException`; awsJson1_x has no malformed-request suite, so the coral behavior is the reference). `serviceutil.DecodeJSON` now returns `protocol.ErrSerialization` instead of `InvalidArgument`, making the backup/cloudtrail/transfer/ecs/kinesis `decodeJSON*` variants true clones — delete them in their Track 2 per-service passes.
