...
| Placement | Every group names at least one of `AvailabilityZones` and `VPCZoneIdentifier`; a group with neither is refused. Launches round-robin across `VPCZoneIdentifier`'s subnets, each instance landing in its own subnet's zone, or across `AvailabilityZones` for a group with no subnets. A group given only subnets takes its zones from them |
...
| Neither `AvailabilityZones` nor `VPCZoneIdentifier` | `CreateAutoScalingGroup`, and any `UpdateAutoScalingGroup` that would leave a group with neither | `400 ValidationError`, as on AWS — `At least one Availability Zone or VPC Subnet is required.` A group with no placement has nowhere to launch |
