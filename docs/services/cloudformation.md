...
- **S3 bucket sub-resources.** `AWS::S3::Bucket` translates `LifecycleConfiguration`, `VersioningConfiguration`, `NotificationConfiguration`, `BucketEncryption`, `Tags`, `CorsConfiguration`, and `WebsiteConfiguration` into the existing S3 REST-XML APIs. Updates keep the bucket and replace only changed configurations; removal deletes the configuration, clears notifications, or suspends versioning as AWS's irreversible versioning state requires. Unknown properties and fields the S3 API model cannot preserve fail the resource instead of being silently dropped.
