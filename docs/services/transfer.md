...
| --------------------------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Data plane                                                | SFTP, FTPS and FTP are served                                              | No file-transfer protocol is served, so no client can log in and no file moves                                                                          |
| S3 or EFS binding                                         | `HomeDirectory` maps to a bucket or file system                            | A stored string that maps to nothing                                                                                                                    |
| Server fields                                             | Stored and applied                                                         | `Protocols`, `EndpointDetails`, `LoggingRole`, `Domain` and `SecurityPolicyName` are discarded; only `EndpointType` and `IdentityProviderType` are kept |
| User fields                                               | Stored and applied                                                         | `HomeDirectoryType`, `HomeDirectoryMappings`, `PosixProfile` and `SshPublicKeyBody` are discarded; only `Role`, `HomeDirectory` and `Policy` are kept   |
| Authentication                                            | SSH public keys, service-managed credentials and custom identity providers | None of it is exercised                                                                                                                                 |
| Endpoint address                                          | `DescribeServer` reports a hostname, VPC endpoint or Elastic IP            | It reports none of them                                                                                                                                 |
| Server ids                                                | Random                                                                     | Sequential — `s-00000001`, `s-00000002`, …                                                                                                              |
| Agreements, certificates, connectors, profiles, workflows | Full API                                                                   | Not emulated, and their ARNs are refused by the tag operations                                                                                          |
