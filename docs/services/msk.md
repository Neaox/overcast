...
(default 49092). A readiness check sends Kafka's `ApiVersions` request to port 9092 and requires a
broker's answer before transitioning the cluster to "ACTIVE" — a dial is not enough, since Docker's
published port accepts connections as soon as the port proxy is up. A broker that never answers
leaves the cluster in `FAILED` rather than `CREATING`, with the reason in `stateInfo`. When Docker
is unavailable, operations are metadata-only and status transitions immediately.
...
| ------------------------------ | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `CreateCluster`                | ✅ Supported   | Docker-backed (Redpanda); async CREATING→ACTIVE once the broker answers `ApiVersions`; a broker that never answers ends in `FAILED` with `stateInfo`; port auto-alloc from MSK_PORT_BASE | [docs](https://docs.aws.amazon.com/msk/latest/developerguide/API_CreateCluster.html)                |
...
| `CreateClusterV2`              | ✅ Supported   | PROVISIONED: same Docker/Redpanda lifecycle as v1; SERVERLESS: metadata-only, immediately ACTIVE; rejects a request naming both or neither                                               | [docs](https://docs.aws.amazon.com/msk/latest/developerguide/API_CreateClusterV2.html)              |
