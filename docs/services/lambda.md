...
- Extension telemetry subscriptions — both the Logs API
  ...
  — support HTTP destinations only, and buffering configuration is accepted
  but not modelled: every record is delivered as its own single-event batch.
  Delivery retries a transport failure with backoff and drops the record only
  when the destination fails every attempt.
...
| `platform.runtimeDone`     | `END`    | `DEBUG` on success, else `WARN`                               | `requestId`, `status`, `tracing`, `metrics`: `durationMs`, `producedBytes`                              |
...
answered (a crash, a timeout). Overcast emits only the subset of the Telemetry
API schema it genuinely observes: the
`errorType` member the invocation records model is never
...
at-least-once: a transport failure is retried with backoff, records are
delivered one per batch (buffering configuration is accepted and not
modelled), and only a destination that fails every attempt loses a record —
reported in Overcast's own log at `WARN`.
