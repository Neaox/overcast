...
- Extension telemetry subscriptions — both the Logs API
  (`PUT /2020-08-15/logs`) and the Telemetry API (`PUT /2022-07-01/telemetry`)
  — support HTTP destinations only. Buffering configuration is honoured with
  AWS's defaults; out-of-range values are clamped to the documented limits
  rather than rejected, because AWS's rejection behaviour is not documented.
- The SnapStart restore records (`platform.restoreStart`,
  `platform.restoreRuntimeDone`, `platform.restoreReport`) are not emitted,
  because SnapStart itself is not emulated. Of the documented
  `platform.runtimeDone` spans, only `responseLatency` is emitted:
  `responseDuration` ends only after the answer has finished streaming
  through the init's unbuffered proxy, and `runtimeOverhead` exists only at
  the runtime's next poll — both after the record is already on its way. A
  dropped run of *frames* on the init's log channel is still reported in
  Overcast's own log, not as a Telemetry record; a dropped *delivery* to a
  subscriber is a `platform.logsDropped` event on that subscriber's stream.
...
`Init Duration`. `tracing` carries the `X-Amzn-Trace-Id` the runtime genuinely
received for the invocation (`spanId` stays absent — Overcast mints no spans).
`platform.runtimeDone`'s `metrics` are measured by the in-container init — the
invocation being handed to the runtime to its answer arriving back at the
init's proxy, with `producedBytes` from the length the runtime declared — so
they are a different, smaller span than `platform.report`'s host-measured
`durationMs`, as AWS's two records are two different measurements. The host's
own measurements are the fallback on the paths where the runtime never
answered (a crash, a timeout). `spans` on `platform.runtimeDone` carries the
`responseLatency` the init measured. Overcast emits only the subset of the
Telemetry API schema it genuinely observes: `errorType` is populated for the
one outcome whose AWS name is documented — a runtime that exited is
`Runtime.ExitError` — while a handler error and a timeout leave it absent,
exactly as AWS's own examples for those statuses do,
...
- `PUT /2020-08-15/logs`
- `PUT /2022-07-01/telemetry` (and the trailing-slash spelling AWS's reference
  constructs)
...
Subscriptions arrive through either surface — the Logs API
(`PUT /2020-08-15/logs`) or the [Telemetry
API](https://docs.aws.amazon.com/lambda/latest/dg/telemetry-api.html)
(`PUT /2022-07-01/telemetry`), the endpoint AWS documents as superseding it
and the one current observability extensions call. As on AWS, an extension
subscribed via one API is refused by the other. The Telemetry API validates
`schemaVersion` (`2022-07-01`, `2022-12-13` or `2025-01-29`; an unknown
version is refused rather than half-honoured) and answers with the documented
`"OK"` body.

Both surfaces deliver HTTP destinations for `platform`, `function` and
`extension` log types. Function stdout/stderr is delivered as `function`
...
events under JSON. From `schemaVersion: 2022-12-13`, a `LogFormat: JSON`
function's log line is embedded in its record as the JSON object it already
...
at-least-once and batched: a batch is cut at the subscription's `maxItems`,
`maxBytes` or `timeoutMs` (AWS's defaults when unspecified; out-of-range
values are clamped to the documented limits), a transport failure is retried
with backoff, and a destination that fails every attempt loses that batch —
and is told: the next batch delivered to it opens with a
`platform.logsDropped` event carrying `reason`, `droppedRecords` and
`droppedBytes`, in the shape AWS documents.
