...
## Choosing `OVERCAST_SERVICES` for your CDK app

By default every service is enabled and you can ignore this section. It matters
when you trim `OVERCAST_SERVICES` — and the intuitive way to trim it, listing
the services your stack creates, does not work.

### `cdk deploy` needs services your stack never mentions

Whatever your stack contains, the deploy workflow above uses STS, S3, SSM, IAM,
and CloudFormation. Bootstrap needs the same set. So the minimum for any CDK
app is:

```bash
OVERCAST_SERVICES=sts,s3,ssm,iam,cloudformation,<your services>
```

A stack that only creates a DynamoDB table still needs all six tokens, not just
`dynamodb`. Omitting them breaks `cdk bootstrap` before your resources are ever
reached.

### Some constructs quietly need a second service

A few constructs depend on a partner service for behaviour rather than for
provisioning. The resource is still created if the partner is missing — the
behaviour is silently skipped:

| If you use | Also enable | Otherwise |
| ---------- | ----------- | --------- |
| `lambda.Function` | `logs` | The function deploys and runs, but writes no invocation logs |
| `lambda.Function` with `Code.fromAsset` | `s3` | Asset upload fails, and code updates don't refresh the warm pool |
| `lambda.Function` with `vpc` | `ec2` | The container starts without VPC attachment |
| `s3.Bucket` with notifications | `sqs` (and `lambda` for Lambda targets) | Notifications are never delivered |
| `ecs.FargateService`, `rds.DatabaseInstance` | `ec2` | Subnet-backed launches don't resolve against VPC state |
| `sns.Topic` with SQS subscriptions | `sqs` | Subscriptions are stored but never fan out |

### Token names don't match CDK module names
...
exactly: an unknown one fails at startup with
`unknown service "..." in OVERCAST_SERVICES`, and a call to a service you left
out returns HTTP 503 `ServiceDisabled`. Through CloudFormation that surfaces as
a failed stack rather than a clear API error, so if a deploy fails on a
resource type listed as supported below, check your token list first.
