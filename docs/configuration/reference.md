...
| `OVERCAST_VPC_EGRESS`            | `open`                 | Whether the containers Overcast starts reach anything outside this machine: `open`, `routed` or `none`. Any other value fails startup — see [Egress modes](../networking.md#egress-modes) |
| `OVERCAST_VPC_EGRESS_POOL`       | `198.18.0.0/16`        | IPv4 range the per-VPC egress networks of `routed` are carved from, one `/24` each. `/8` to `/24`; the default supports 256 VPCs with egress — see [The address-pool ceiling](../networking.md#the-address-pool-ceiling) |
