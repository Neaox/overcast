...
**An EC2 VPC becomes one Docker bridge** named after the VPC, with the subnet taken from its CIDR, and marked `--internal` unless the VPC has an attached internet gateway. That flag only bites when the **control plane** is internal too, since every container sits on both — which is `OVERCAST_CONTROL_PLANE_INTERNAL`'s business: `auto` (the default) decides from the host, `true` and `false` pin it. See [docs/networking.md § Control-plane isolation](../networking.md#control-plane-isolation) and `dataplane.ControlPlaneInternal`. A VPC whose network could not be created reports status `unbacked` and services refuse to place into it; the usual cause is a CIDR collision with a leftover network from an earlier run.
...
- **Whether a VPC with no internet gateway reaches the internet depends on the host.** Its own network is `--internal`, but every container also sits on the control plane, and *that* is internal only when `OVERCAST_CONTROL_PLANE_INTERNAL` resolves to true — under the default `auto`, when Overcast is containerised or on a native Linux daemon. So the same stack egresses on a Docker Desktop host and does not in a container, unless the setting is pinned. The decision and its reason are logged at startup and in `/_overcast/health` under `docker.networks`.
- **A `{network}_control` created by an older Overcast keeps the isolation it was born with.** Docker never applies `--internal` retroactively. Overcast recreates a drifted plane at startup when nothing is attached, and warns and leaves it when something is — so a long-lived machine can lag a version pin until the network is removed.
