...
- **`OVERCAST_CONTROL_PLANE_INTERNAL` is deprecated.** Still honoured, still wins for that one network where it is pinned, and setting it logs a notice naming the mode that means the same thing. It pins one network, and a container takes its default route from whichever of its networks is routable.
