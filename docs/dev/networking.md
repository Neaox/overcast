...
**An EC2 VPC becomes one Docker bridge** named after the VPC, with the subnet taken from its CIDR, and marked `--internal` unless the VPC has an attached internet gateway — though that flag remains partly decorative, because every container also sits on the control plane, which is not internal. Closing that is a separate change: it would take a container in a gateway-less VPC from "no route out but the control plane" to no route out at all, which is correct for a private subnet and a hard break for anything reaching a real endpoint during INIT. A VPC whose network could not be created reports status `unbacked` and services refuse to place into it; the usual cause is a CIDR collision with a leftover network from an earlier run.
