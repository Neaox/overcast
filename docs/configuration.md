...
| `OVERCAST_VPC_EGRESS`            | `open`                 | Whether the containers Overcast starts reach anything outside this machine: `open` (all do), `none` (none do; the Lambda Runtime API still works, and this isolates the default plane too, so functions with no VPC are affected), `routed` (each subnet gets what its route table says — a `0.0.0.0/0` route to an internet or NAT gateway grants egress, no default route withholds it; **run Overcast in a container**, since natively on Docker Desktop it cannot withhold and reports `vpc-egress-not-withheld`). Any other value fails startup rather than falling back to `open` — see [Egress modes](./networking.md#egress-modes) |
