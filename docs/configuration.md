...
| `OVERCAST_VPC_EGRESS`            | `open`                 | Whether the containers Overcast starts reach anything outside this machine: `open` (all do), `none` (none do; the Lambda Runtime API still works), `routed` (each subnet gets what its route table says — a `0.0.0.0/0` route to an internet or NAT gateway grants egress, no default route withholds it) — see [Egress modes](./networking.md#egress-modes) |
