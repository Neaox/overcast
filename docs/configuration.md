...
| `OVERCAST_CONTROL_PLANE_INTERNAL` | `auto`                | Whether `<name>_control` is created `--internal`: `auto` (decided from the host), `true`, `false`. This is what decides whether compute in a VPC with no internet gateway can reach the internet — see [Control-plane isolation](./networking.md#control-plane-isolation) |
