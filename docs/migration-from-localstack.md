...
| `GATEWAY_LISTEN`  | `OVERCAST_LISTEN` + `OVERCAST_PORT`      | Bind address, split into two variables. Default: `0.0.0.0` in a container, `127.0.0.1` natively — an explicit value always wins over either default. (Renamed from `OVERCAST_HOST`, which has been removed — a leftover `OVERCAST_HOST` fails at startup naming `OVERCAST_LISTEN` as the replacement, rather than being silently ignored) |
