...
> **Status:** Phases 1 and 2 are complete, committed, and rebased onto `main` (branch `feat/storage-stabilization`, worktree `E:\dev\overcast-storage-stabilization`, pushed to `origin`). **Phase 3 has not been started — pick up here next session.** See "Phase 1 completion notes" and "Phase 2 completion notes" below for what shipped and what was found along the way, and "Rebase onto main" for how the two real conflicts were resolved. The Phase 3 item table below (with per-item guidance already written) is the starting point for the next session; no Phase 3 code exists yet. One outstanding decision for whoever resumes: Phase 1's `docs/storage-plan.md` "Settled decisions" note a real, separate bug found during Phase 1 (per-service override config names not always matching `NamespacedStore` routing prefixes, e.g. `"cloudformation"` vs `"cfn"`) — still unfixed, not itemized as its own Phase 3 task, worth deciding whether to add one.
...
| 3.11 | **Kinesis region-scoping check** | `kinesis:records` keys look region-less (`<stream>/<shard>/<seqNo>`, [internal/services/kinesis/store.go:24](../internal/services/kinesis/store.go)) unlike SQS/logs. Verify whether two regions' streams collide; fix keys if so (correctness, not perf). |
