...
- **3.14 (config-name/routing-prefix mismatch) — trigger: none, it's ready now.** Unlike the three above, this is a correctness bug with a known fix shape, not a "measure first" item — see its full writeup in the table row above. Pick it up whenever convenient; no benchmark or further investigation is a prerequisite.
