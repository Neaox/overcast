...
| `A VPC network does not match its internet gateway` | Overcast could not bring a VPC's Docker network to the isolation its gateway state calls for. Containers in that VPC have the reachability of the gateway state *before* the one `DescribeInternetGateways` reports. It retries at the next restart — see [EC2 limitations § Internet gateways and isolation](./services/ec2/limitations.md#internet-gateways-and-isolation). |
| `OVERCAST_VPC_EGRESS=none cannot withhold egress on this host` | The control plane had to stay routable, so containers keep a route out and the stack is not hermetic. Expected on Docker Desktop with Overcast on the host — see [Egress modes](./networking.md#egress-modes). |
| `No Runtime API address is reachable from a container` | No candidate address a Lambda container could dial answered, so every invocation will fail during INIT. Usually a host firewall blocking a freshly built binary — see [Containers cannot reach the Runtime API](./services/lambda/troubleshooting.md#containers-cannot-reach-the-runtime-api). |
