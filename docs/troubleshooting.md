...
| `OVERCAST_VPC_EGRESS=none cannot withhold egress on this host` | The control plane had to stay routable, so containers keep a route out and the stack is not hermetic. Expected on Docker Desktop with Overcast on the host — see [Egress modes](./networking/egress.md). The title names whichever mode you set: under `routed` it reads `OVERCAST_VPC_EGRESS=routed cannot withhold egress on this host`, and the detail then also covers VPC placement (below). |
