...
| `OVERCAST_VPC_EGRESS=none cannot withhold egress on this host` | The control plane had to stay routable, so containers keep a route out and the stack is not hermetic. Expected on Docker Desktop with Overcast on the host — see [Egress modes](./networking.md#egress-modes). The title names whichever mode you set: under `routed` it reads `OVERCAST_VPC_EGRESS=routed cannot withhold egress on this host`, and the detail then also covers VPC placement (below). |
| `OVERCAST_VPC_EGRESS=routed cannot withhold egress on this host` | The same control-plane limit, plus one only `routed` has: where Overcast's DNS resolver cannot start, a VPC-placed container also joins the routable shared plane, so its subnet's route table decides nothing and a missing NAT gateway goes uncaught. Run Overcast in a container, or against a native Linux daemon — see [`routed`](./networking.md#routed-egress-from-your-route-tables). |
