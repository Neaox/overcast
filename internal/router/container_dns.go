...
//
// No zone address is set. Overcast attaches to overcast_lambda and overcast_ecs
// by default and so has an address on each; the server picks the one reachable
// from whichever container is asking.
func startContainerDNS(cfg *config.Config, logger *zap.Logger) (stop func()) {
