...
func init() {
	capabilities.Default.Register(
		capabilities.Capability{Service: "appconfigdata", Operation: "StartConfigurationSession", Category: "Sessions",
			Status: capabilities.StatusWIP, Notes: "Starts a polling session; returns `InitialConfigurationToken`; Overcast does not serve the binding AWS models, so no SDK reaches it (#855)",
		...
		capabilities.Capability{Service: "appconfigdata", Operation: "GetLatestConfiguration", Category: "Sessions",
			Status: capabilities.StatusWIP, Notes: "Returns current config content; empty body when unchanged since last poll; Overcast does not serve the binding AWS models, so no SDK reaches it (#855)",
