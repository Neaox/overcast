...
func init() {
	capabilities.Default.Register(
		...
		// Roles
		capabilities.Capability{Service: "iam", Operation: "CreateRole", Category: "Roles", Status: capabilities.StatusSupported, Notes: "Inline `Tags` applied at creation; the `AssumeRolePolicyDocument` is parsed, and a malformed one rejected with `MalformedPolicyDocument` (400) — see `CreatePolicy` for what that check does and does not cover"},
