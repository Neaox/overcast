...
// build rather than failing in it — same convention as
// index_store_sqlite_test.go in this package. (item_store_test.go and
// index_store_test.go are untagged: their fixture drops the SQL backend under
// nosqlite so the memory half of each parity test still runs — see
// newTestItemBackends.)
package dynamodb
