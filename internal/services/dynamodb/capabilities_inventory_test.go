...
func TestCapabilities_MatchDispatchInventory(t *testing.T) {
	...
	caps := capabilities.Default.ForService("dynamodb")
	capsSet := make(map[string]struct{}, len(caps))
	// docOnly holds capabilities that document AWS operations Overcast does not
	// dispatch (streams interop documented on the streams service, unsupported
	// operations without explicit stubs). They are metadata only and are exempt
	// from the dispatch cross-check, mirroring capgen.
