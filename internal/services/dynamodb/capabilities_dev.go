...
func init() {
	capabilities.Default.Register(
		...
		capabilities.Capability{Service: "dynamodb", Operation: "DescribeTimeToLive", Category: "Table management", Status: capabilities.StatusSupported, Notes: "Reports all four `TimeToLiveStatus` values — a change is `ENABLING`/`DISABLING` until it settles; `AttributeName` is omitted only once the table has settled on `DISABLED`"},
		capabilities.Capability{Service: "dynamodb", Operation: "UpdateTimeToLive", Category: "Table management", Status: capabilities.StatusSupported, Notes: "TTL-based item expiry, with the sweeper deleting expired items hourly once the status reaches `ENABLED`; the change is asynchronous as on AWS, and both members of `TimeToLiveSpecification` are required (`AttributeName` 1..255). A second `UpdateTimeToLive` inside the transition window, re-enabling an `ENABLED` table (renaming the attribute included) and disabling a `DISABLED` one are each a `ValidationException`, so the attribute name only changes by disabling first. Overcast settles in 30 seconds where AWS takes up to an hour"},
		// Item operations
		...
		capabilities.Capability{Service: "dynamodb", Operation: "GetItem", Category: "Item operations", Status: capabilities.StatusSupported, Notes: "Includes `ProjectionExpression`; a `Key` must name exactly the key schema's attributes with their declared types; records SuccessfulRequestLatency, ConsumedReadCapacityUnits, UserErrors/SystemErrors; a reserved word used as a bare attribute name in an expression is rejected, as AWS does — reach it through `ExpressionAttributeNames`"},
