...
// *sql.Tx-wrapped atomicity. Run against every backend this build provides
// (newTestItemBackends, shared with item_store_test.go) so a fix to one can't
...
// The *sql.Tx rollback test for putWithIndexMutations lives in
// index_store_sqlite_test.go, which carries //go:build !nosqlite because it is
// wholly about sqlItemBackend's transaction handling.
