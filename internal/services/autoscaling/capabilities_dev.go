...
func init() {
	capabilities.Default.RegisterForService(serviceName,
		...
		capabilities.Capability{Operation: "UpdateAutoScalingGroup", Status: capabilities.StatusSupported, Notes: "Min/max/desired changes re-converge the group; desired outside min/max, VPCZoneIdentifier subnets outside AvailabilityZones, and an update leaving the group with neither AvailabilityZones nor VPCZoneIdentifier, are AWS's ValidationError. Setting either launch source clears the other"},
