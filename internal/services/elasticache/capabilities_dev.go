...
func init() {
	capabilities.Default.Register(
		...
		capabilities.Capability{Service: "elasticache", Operation: "CreateReplicationGroup", Category: "General", Status: capabilities.StatusSupported, Notes: "Docker-backed (single primary node); async creating→available via TCP health check; `CacheSubnetGroupName` places the group in that subnet group's VPC"},
		capabilities.Capability{Service: "elasticache", Operation: "CreateServerlessCache", Category: "General", Status: capabilities.StatusSupported, Notes: "Docker-backed (redis/valkey/memcached); async creating→available via TCP health check; CloudFormation ServerlessCache supported"},
