...
// See docs/services/kms.md for the support matrix.
//
// Wire protocol: JSON (X-Amz-Target: TrentService.*)
// Symmetric keys use AES-256-GCM for Encrypt/Decrypt.
// RSA_2048 SIGN_VERIFY keys use RSASSA_PKCS1_V1_5_SHA_256.
package kms
...
func (s *Service) TargetPrefix() string { return "TrentService." }
...
func (s *Service) Dispatch(w http.ResponseWriter, r *http.Request) {
	if c, opName := codec.FromContext(r.Context()); c != nil && opName != "" {
		if !codec.Supports(s.SupportedProtocols(), c) {
			...
			c.WriteError(w, r, &protocol.AWSError{
				...
				Message:    "KMS does not support wire protocol " + c.Name() + ".",
				HTTPStatus: http.StatusUnsupportedMediaType,
			})
			return
		}
		// Preserve AWS JSON 1.1 on the existing handler path until JSON
		// wire-byte goldens cover KMS. CBOR uses typed operations.
	...
	suffix := strings.TrimPrefix(r.Header.Get("X-Amz-Target"), "TrentService.")
