...
func init() {
	capabilities.Default.RegisterForService(serviceName,
		capabilities.Capability{Operation: "CreateLoadBalancer", Category: "Load Balancers", Status: capabilities.StatusSupported, Notes: "Threads Type, Scheme, IpAddressType and Tags; requires at least one Subnets or SubnetMappings member, which is validated and then discarded — Availability Zones are not modelled"},
		capabilities.Capability{Operation: "DescribeLoadBalancers", Category: "Load Balancers", Status: capabilities.StatusSupported, Notes: "An unresolvable LoadBalancerArns or Names entry raises LoadBalancerNotFound and a value that is not a load balancer ARN raises ValidationError; naming neither lists the region"},
		capabilities.Capability{Operation: "DeleteLoadBalancer", Category: "Load Balancers", Status: capabilities.StatusSupported},
		capabilities.Capability{Operation: "CreateTargetGroup", Category: "Target Groups", Status: capabilities.StatusSupported, Notes: "Threads TargetType, ProtocolVersion, IpAddressType, the HealthCheck* family, Matcher and Tags; validates Protocol against the documented enum and Port against 1-65535; health checks are stored and echoed but not evaluated against targets"},
		capabilities.Capability{Operation: "DescribeTargetGroups", Category: "Target Groups", Status: capabilities.StatusSupported, Notes: "An unresolvable TargetGroupArns or Names entry raises TargetGroupNotFound and a value that is not a target group ARN raises ValidationError; naming neither lists the region"},
		...
		capabilities.Capability{Operation: "DescribeListeners", Category: "Listeners", Status: capabilities.StatusSupported, Notes: "An unresolvable ListenerArns entry raises ListenerNotFound and an unresolvable LoadBalancerArn raises LoadBalancerNotFound; naming neither lists the region"},
