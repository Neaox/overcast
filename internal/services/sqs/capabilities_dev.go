...
func init() {
	capabilities.Default.Register(
		...
		capabilities.Capability{Service: "sqs", Operation: "GetQueueAttributes", Category: "Queue management",
			Status: capabilities.StatusSupported, Notes: "All standard attributes; All wildcard supported; ApproximateNumberOfMessages(NotVisible) reflects the same counts a background sampler also publishes to CloudWatch every minute as ApproximateNumberOfMessagesVisible/NotVisible/Delayed (service-metrics-platform.md phase 2), whether or not the queue has traffic"},
		...
		capabilities.Capability{Service: "sqs", Operation: "SendMessage", Category: "Message operations",
			Status: capabilities.StatusSupported, Notes: "DelaySeconds, MessageAttributes supported; records AWS/SQS CloudWatch metrics NumberOfMessagesSent and SentMessageSize (service-metrics-platform.md phase 2), skipped for a FIFO content-based-deduplication resend since no new message is enqueued"},
