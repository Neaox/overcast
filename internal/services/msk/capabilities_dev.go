...
func init() {
	capabilities.Default.Register(
		...
		capabilities.Capability{Service: "msk", Operation: "CreateClusterV2", Category: "Clusters",
			Status: capabilities.StatusWIP, Notes: "PROVISIONED: same Docker/Redpanda lifecycle as v1; SERVERLESS: metadata-only, immediately ACTIVE; Overcast does not serve the binding AWS models, so no SDK reaches it (#859)"},
		capabilities.Capability{Service: "msk", Operation: "DescribeClusterV2", Category: "Clusters",
			Status: capabilities.StatusWIP, Notes: "Returns v2 shape with `clusterType` and `provisioned`/`serverless` sub-object; Overcast does not serve the binding AWS models, so no SDK reaches it (#859)"},
