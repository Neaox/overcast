...
// real Logs operation Overcast has not implemented gets an honest 501, in the
...
func TestDispatch_unimplementedVsUnknownOperation(t *testing.T) {
	// Given: PutDestination is a real, documented Logs operation
	// (internal/awsapi/manifest.gen.go, under cloudwatch-logs) with no
	// handler here.
	const unimplemented, unknown = "PutDestination", "NotALogsOperation"
	...
	if !awsapi.HasOperation(awsapiService, unimplemented) || awsapi.HasOperation(awsapiService, unknown) {
