...
// newTestBackends returns one eventBackend per implementation available in
// this build, keyed by the label used for its t.Run subtest, so tests can run
// the same assertions against every one of them — the memory-mode parity
// requirement from docs/plans/storage-plan.md 2.3.
//
// "sql" is a real sqlEventBackend over a temp-dir-rooted *state.HybridStore,
...
// memory-only rather than the whole file being tagged out — the memory half
// of every parity test below still works there and is worth keeping. See
...
// choice in a build with SQLite compiled in.
