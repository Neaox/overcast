...
| `docs/*.md` | users | yes — run `make docs-lint` | applies |
...
- **Editing a published doc means running `make docs-lint`.** There is nothing to regenerate or commit beyond the doc itself — the console derives its navigation and search index from the embedded docs at runtime. `docs/dev/` and `docs/plans/` are neither published nor indexed.
- **Every anchor is GitHub's slug — published docs, `docs/dev/`, `docs/plans/`, `AGENTS.md`, the root README alike.** The console, the public site and github.com all id headings the way github-slugger does: punctuation is dropped, not turned into a hyphen, and each space becomes one, so nothing collapses — "HTTPS / TLS" is `#https--tls`, "Data dir placement — avoid …" is `#data-dir-placement--avoid-…`, "`Fn::GetAtt` support" is `#fngetatt-support`, and a repeated heading is `#setup-1`, then `#setup-2`. `make docs-check` validates the published side against the real heading ids and names the id you meant.
...
- [ ] `make docs-lint` run if a published doc changed; changelog question answered.
