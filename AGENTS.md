...
- A bare checkout still builds, vets and tests. `go build ./...` and `go vet ./...` need nothing from you. `go test ./...` needs the artefacts, and the test bootstraps take care of it: `internal/services/lambda`'s `TestMain` and `tests/integration/lambda`'s `requireLambdaInit` build anything missing once per test binary — and, because an embed is baked at *compile* time and a fresh checkout's test binary was compiled before the artefacts existed, they point `initbin.EnvDistDir` (`OVERCAST_LAMBDA_INIT_DIST`) at the freshly built files so the very first `go test` passes, not the second. That env override is also how to run a locally patched init without relinking Overcast; unset, production reads only the embed.
