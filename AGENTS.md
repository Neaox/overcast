...
- **Reproduce the AWS operation coverage CI job with `make aws-models-check`.** It validates, without network access: the committed manifest and its runtime ownership indexes; protocol identifiers; that every declared operation name is real in AWS — **including a `DocOnly` row named like an operation**; that a REST-bound operation is **registered at the HTTP method and URI the model binds it to**; that an operation is reachable over every protocol its service answers on; that a row declaring Supported is one a client can actually call; and that the wire facts a service states about itself agree with the model — its `TargetPrefix()`, its `PathPrefixes()` claim, and the Query API versions and service aliases in `internal/awsapi`. With a pinned checkout, set `AWS_MODELS_DIR` to add the same byte-for-byte regeneration check used by the scheduled model-refresh workflow.
