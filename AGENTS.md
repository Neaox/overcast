...
- **Reproduce the AWS operation coverage CI job with `make aws-models-check`.** It validates the committed manifest, runtime ownership indexes, protocol identifiers, and capability-to-model alignment without network access. With a pinned checkout, set `AWS_MODELS_DIR` to add the same byte-for-byte regeneration check used by the scheduled model-refresh workflow.

  **It does not check that an operation is served where AWS serves it.** The wording here used to say "router coverage", which reads as exactly that guarantee and is not one. What the router test actually asks is whether an *unclaimed* operation falls through to S3's wildcard — its fixture registers only S3 and its pass condition is a `501`, so a 501 is the success case. An operation implemented at an invented path is indistinguishable from one that is deliberately unimplemented. That is how EventBridge Scheduler stayed unreachable for 33 releases, and why seven more services were found the same way (see `docs/plans/route-reachability-audit.md`). The manifest records `HTTPMethod` and `URI` for every operation and nothing compares them to the registered routes — closing that is tracked separately.
