...
cat <<'JSON'
{
  "hookSpecificOutput": {
    ...
    "additionalContext": "This looks like a hand-rolled CI poll loop around `gh pr checks`. Don't. `gh pr checks` already has `--watch --fail-fast`, and `scripts/pr-wait.sh <pr>` wraps it: it blocks until every check settles, prints only the non-passing lines plus a tally, re-reads mergeStateStatus, and exits 0/1/2/8 (passed / failed / no checks will run / pending). In a top-level session run it with the Bash tool and run_in_background: true, so you get exactly ONE completion notification instead of a message per interval. A poll loop costs a request per tick, fires whether or not anything changed, and produces a play-by-play that changes no decision — the user has objected to precisely this. SUBAGENTS: do not background it and stop. You are re-invoked only if a tracked child is still running when your turn ends, and pr-wait often exits in seconds (bad arg, gh auth hiccup, a PR that already settled, its own exit 2) — the child dies first, no wake arrives, and you are stranded mid-task. Run it in the FOREGROUND, or skip waiting altogether: once `gh pr merge <n> --squash --auto` is armed, auto-merge finishes the PR unattended, so your terminal state is 'auto-merge armed, one foreground `gh pr checks <n>` shows no FAILED required check → report and exit'. Also: never pipe an exit-code-bearing gh command into another, since the pipeline reports the last command's status (that is how PR #410 merged over a failing compat check). See AGENTS.md § Waiting on CI. If you genuinely need a one-shot read of current state rather than a wait, drop the loop and just call `gh pr checks <n>` once."
  }
}
JSON
