...
rather than a hand-written method. The backend is the generated classes
`groups/Scenarios*Gen.java`, which `cmd/compatgen` writes from the scenario IR
and `mvn package` compiles like any other source — see
...
java-sdk/
  ...
  src/main/java/io/overcast/compat/
    ...
    registry/
      ...
      ScenarioBackend.java ← extension point for executing generated groups
    ...
    groups/
      ...
      ScenariosGen.java, Scenarios<Service>Gen.java  ← GENERATED by cmd/compatgen
  ...
  src/test/java/io/overcast/compat/
    ...
    scenario/                          ← the generated groups' runtime, offline
```

**One class per AWS service**, all implementing `ServiceGroup`. See
[AGENTS.md](AGENTS.md) for the exact shape and how to add one — and for why the
`Scenarios*Gen.java` classes beside them are generator output that must never be
hand-edited.
