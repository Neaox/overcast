...
public sealed class KmsGroup(AwsClients clients) : IServiceGroup
{
    ...
    {
        ["kms-keys:CreateKey"] = CreateKeyAsync,
        ["kms-keys:DescribeKey"] = DescribeKeyAsync,
        ["kms-keys:PutKeyPolicy"] = PutKeyPolicyAsync,
        ["kms-keys:CreateKmsAlias"] = CreateKmsAliasAsync,
        ["kms-keys:ListKmsAliases"] = ListKmsAliasesAsync,
        ["kms-keys:ListKeys"] = ListKeysAsync,
        ["kms-keys:DisableKey"] = DisableKeyAsync,
        ["kms-keys:EnableKey"] = EnableKeyAsync,
        ["kms-keys:ScheduleKeyDeletion"] = ScheduleKeyDeletionAsync,
        ["kms-keys:CancelKeyDeletion"] = CancelKeyDeletionAsync,
        ["kms-keys:TagKMSResource"] = TagKMSResourceAsync,
        ["kms-keys:ListKMSResourceTags"] = ListKMSResourceTagsAsync,
        ["kms-keys:UntagKMSResource"] = UntagKMSResourceAsync,
        ["kms-crypto:Encrypt"] = EncryptAsync,
        ["kms-crypto:Decrypt"] = DecryptAsync,
        ["kms-crypto:GenerateDataKey"] = GenerateDataKeyAsync,
        ["kms-crypto:GenerateDataKeyWithoutPlaintext"] = GenerateDataKeyWithoutPlaintextAsync,
        ["kms-asymmetric:Sign"] = SignAsync,
        ["kms-asymmetric:Verify"] = VerifyAsync,
    };
