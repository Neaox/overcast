...
compat/suites/go-sdk/
  ...
  internal/
    ...
    groups/
      ...
      scenarios_gen.go  scenarios_<service>_gen.go
                       ← generated by cmd/compatgen; never edited by hand
    scenario/          ← the runtime the generated groups call into
```
...
## Generated groups — emitted source, not an interpreter
...
A **generated** group (one with `"generated": true`) comes from the scenario
IR at `compat/model/scenarios/<service>.json`. The three interpreter suites
execute that file at run time. This one cannot: the AWS SDK for Go v2 has no
public dynamic-dispatch API, and reaching into smithy-go's marshaller layer to
fake one is rejected by
[the plan](../../../docs/plans/compat-coverage-modelgen.md) §3.2 — the reason
for running eight suites is that each exercises its own real typed
serialization path. So `cmd/compatgen` emits Go instead:

```
internal/groups/
  scenarios_gen.go              ← generated: the index All() appends
  scenarios_sqs_gen.go          ← generated: one file per service
  scenarios_organizations_gen.go
internal/scenario/              ← hand-written: the semantics, written once
```

Each emitted test function builds a real typed input struct and calls a real
client method, and registers under the ordinary group-qualified impl key. So
`ScenarioBackend` **stays nil for this suite**, and deliberately: it is the
hook an interpreter registers to resolve a test with *no* impl, and a
generated-source backend has an impl for every test. The loader's
generated-no-backend failure therefore never fires for a group this suite can
run — its rule is "no impl and no backend", and the emitted impls are
registered.

### Working on them

- **Never edit `scenarios_*_gen.go`.** Change the recipe under
  `compat/model/recipes/`, or the emitter in `cmd/compatgen/emit_go.go`, and
  run `make generate-compat-model`. `make compat-model-check` proves the
  committed files are byte-identical to the generator's output.
- **The semantics belong in `internal/scenario`, never in emitted source.**
  The context bag, `$name`/`$ref`, the closed check set, error matching,
  `eventually` and the six-field failure message are written once there and
  held to [compat/model/README.md](../../model/README.md), which is normative:
  where that page and this package disagree, this package is wrong. The shared
  error-matching fixtures under `compat/model/testdata/errors/` run through
  the real matcher in `errorfixtures_test.go`.
- **A member is assigned through the field's address** —
  `b.Set("QueueUrl", &in.QueueUrl, scenario.Ref("queue.url"))`, not
  `aws.String`. Whether smithy-go made a member a pointer or a value is not
  derivable from the pinned shape snapshot, which is generated from a
  different revision of the AWS model than the vendored SDK; for SQS's
  `ReceiveMessage` the two already disagree about three members the pilot
  sends. `internal/scenario/document.go` carries the full reasoning.
- **A generated group builds its own client** from `clients.Config()`, cached
  behind a `sync.Once`, rather than adding an accessor to `internal/clients`
  for every service the generator learns to cover. That is the one exception
  to "never construct SDK clients in a test function"; it holds unchanged for
  hand-written groups.
- **A new generated service needs its SDK module in `go.mod`.** Pick a version
  whose own `go.mod` requires the `aws-sdk-go-v2` core version already pinned
  here, so adding a service does not bump the core SDK under every other
  suite's tests; Renovate moves them together afterwards.
...
- Never construct SDK clients directly in test functions (the generated
  groups' own client is the documented exception — see above).
